...
class Scan extends BaseController
{
   ...
   public function absenMasuk($type, $result)
   {
      ...
      switch ($type) {
         case TipeUser::Guru:
            ...
            $kirimwa  = $this->kirimwa_wabiz($pesankirim, $gurune->no_hp);
         ...
         case TipeUser::Siswa:
            ...
            // $kirimwa  = $this->kirimwasender($pesankirim, $siswa['no_hp']);
             $kirimwa  = $this->kirimwa_wabiz($pesankirim, $siswa['no_hp']);
   ...
   public function registerstore()
   {
      ...
      if ($result) {
        ...
        //  $this->kirimwasender($pesan, $this->request->getVar('no_hp'));
         $this->kirimwa_wabiz($pesan, $this->request->getVar('no_hp'));
   ...
   public function teskirim(){
      $pesan = '*Assalamualaikum Wr., Wb.*%0a %0a kirim dari server Absensi MTSN 2 Kota Batam';
      $target = '087710248484';

      $kirim = $this->kirimwa_wabiz($pesan, $target);
    ...
    public function kirimwa_wabiz($pesan, $target, $gambar=""){
        $querset = $this->db->table('pengaturan')->where('id',1)->get()->getRow();
        $appkey = $querset->app_key;
        $no_server = $querset->auth_key;
        ...
        if($gambar == "" || empty($gambar)){
            $params = array(
                'api_key' => $appkey,
                'sender' => $no_server,
                'number' => $this->format_nomor($target),
